\begin{code}
module ParallelReduction3 where
...
data _⇉_ : Λ → Λ → Set where
  ...
  ⇉β :  (x y : V){M M' N N' P : Λ} → y # ƛ x M
        → M [ x / v y ] ⇉ M' → N ⇉ N' → M' [ y / N' ] ∼α P     
        → ƛ x M · N ⇉ P
\end{code}
...
-- Lemma 7
postulate lemma⇉* : {x : V}{M N : Λ} → x * N → M ⇉ N → x * M 
-- lemma⇉*  x*x            (⇉v x)                 
--   = x*x
-- lemma⇉*  (*·l x*N)     (⇉· M⇉N M'⇉N')     
--   = *·l (lemma⇉* x*N M⇉N)
...
--   = *ƛ z*M x≢z
-- lemma⇉*  {z} 
--          z*M'[y/N']    (⇉β x y {M} {M'} {N} {N'} y#ƛxM M[x/y]⇉M' N⇉N' _)  
--   = {!!}
