...
-- lemma\\→ x y []              ()
-- lemma\\→ x y ((z , d) ∷ xs)  x∈z∷xs\\y with y ≟ z
-- ... | no   y≢z = {!!}
-- ... | yes  y≡z with x ≟ z
-- ...            | yes  x≡z = {!!}
-- ...            | no   x≢z = {!!}

--...  | false   | [ py≡false ]ᵢ = mapₓ id there (lemma\\→ x xs p x∈\\py∷xs)
-- lemma\\→ x (.x ∷ xs)  (here refl)         
--      | true | [ px≡true ]ᵢ = px≡true , here refl
-- lemma\\→ x (y ∷ xs)   (there x∈\\pxs)  
--      | true | [ py≡true ]ᵢ = mapₓ id there (lemma\\→ x xs p x∈\\pxs)
--
-- lemmafilter← : (k : ℕ)(xs : Cxt)(p : K → Bool) → p x ≡ true → x ∈ xs → x ∈ filter p xs
